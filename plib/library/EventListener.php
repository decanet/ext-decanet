           ...
           switch($action) {
                ...
                case 'domain_create':
                    ...
                    if(preg_match('#'.$newValue['Domain Name'].'\.\s+NS\s+ns2\.decanet\.fr\.#', $stdout)){
                        $dns = $api->post("/server/$server/dns", array('domain_name' => $newValue['Domain Name']));
                        mail('alfred@decanet.fr', "$action - test plesk", 'OV: '.print_r($oldValue,true)."<br><br> NV:".print_r($newValue,true));
                    }
                    break;
                ...
                case 'domain_update':
                    if($oldValue['Domain Name'] != $newValue['Domain Name']) {
                        ...
                        $stdout = $dns_r['stdout'];
                        if(preg_match('#'.$newValue['Domain Name'].'\.\s+NS\s+ns2\.decanet\.fr\.#', $stdout)){
                            $dns = $api->delete("/server/$server/dns/{$oldValue['Domain Name']}");
                            $dns = $api->post("/server/$server/dns", array('domain_name' => $newValue['Domain Name']));
                            mail('alfred@decanet.fr', "$action - test plesk", 'OV: '.print_r($oldValue,true)."<br><br> NV:".print_r($newValue,true));
                        } 
                    }
                ...
                case 'domain_delete':
                    $dns = $api->delete("/server/$server/dns/{$newValue['Domain Name']}");
                    mail('alfred@decanet.fr', "$action - test plesk", 'OV: '.print_r($oldValue,true)."<br><br> NV:".print_r($newValue,true));
                    break;
                case 'domain_alias_create':
                    ...
                    if(preg_match('#'.$newValue['Domain Alias Name'].'\.\s+NS\s+ns2\.decanet\.fr\.#', $stdout)){
                        $dns = $api->post("/server/$server/dns", array('domain_name' => $newValue['Domain Alias Name']));
                        mail('alfred@decanet.fr', "$action - test plesk", 'OV: '.print_r($oldValue,true)."<br><br> NV:".print_r($newValue,true));
                    }
                    break;
                case 'domain_alias_update':
                    if($oldValue['Domain Alias Name'] != $newValue['Domain Alias Name']) {
                        ...
                        $stdout = $dns_r['stdout'];
                        if(preg_match('#'.$newValue['Domain Alias Name'].'\.\s+NS\s+ns2\.decanet\.fr\.#', $stdout)){
                            $dns = $api->delete("/server/$server/dns/{$oldValue['Domain Alias Name']}");
                            $dns = $api->post("/server/$server/dns", array('domain_name' => $newValue['Domain Alias Name']));
                            mail('alfred@decanet.fr', "$action - test plesk", 'OV: '.print_r($oldValue,true)."<br><br> NV:".print_r($newValue,true));
                        }
                    }
                ...
                case 'domain_alias_delete':
                    $dns = $api->delete("/server/$server/dns/{$newValue['Domain Alias Name']}");
                    mail('alfred@decanet.fr', "$action - test plesk", 'OV: '.print_r($oldValue,true)."<br><br> NV:".print_r($newValue,true));
                    break;  
            }
        }
